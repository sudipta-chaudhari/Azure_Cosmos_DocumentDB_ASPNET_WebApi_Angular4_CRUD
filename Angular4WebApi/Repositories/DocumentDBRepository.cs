...
namespace Angular4WebApi.Repositories
{
    public static class DocumentDBRepository<T> where T : class
    {
        ...
        public static async Task<ResourceResponse<Document>> DeleteItemAsync(string id)
        {
            try
            {
                return await _documentClient.DeleteDocumentAsync(UriFactory.CreateDocumentUri(DatabaseId, CollectionId, id));
            }
            catch (Exception)
            {
                throw;
